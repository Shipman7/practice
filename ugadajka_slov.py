from random import *
gob_spisok = ['год', 'человек', 'время', 'дело', 'жизнь', 'день', 'рука', 'раз', 'работа', 'слово', 'место', 'лицо',
...
def play(word):
    ...
    while not guessed and tries > 0:
        a = input("Напишите букву или слово целиком\n").upper()
        if a.isalpha() and len(a) == 1:
            if a in guessed_letters:
                input('Вы уже называли эту букву\n')
            elif a in word:
                guessed_letters.append(a)
                print("Отличная работа, Вы угадали букву", a, "! Так держать!")
                sp_word = list(word_completion)
                ind = [i for i in range(len(word)) if word[i] == a]
                for j in ind:
                    sp_word[j] = a
                word_completion = "".join(sp_word)
                if "_" not in word_completion:
                    guessed = True
            elif a not in word:
                guessed_letters.append(a)
                tries -= 1
                print("Вы не угадали, в слове нет буквы ", a)
        elif a.isalpha() and len(a) == len(word):
            if a in guessed_words:
                input('Вы уже называли слово,', a, ' напишите другое?\n')            
            elif a == word:
                guessed = True
                ...
                tries -= 1
                print("Вы не угадали, это неверное слово")
        else:
            input('А может быть все-таки введем русскую букву или слово?\n')
        print(display_hangman(tries))
        print(word_completion)
        print()
    ...
    if guessed:
            ...
            print("К сожалению, у Вас закончились все попытки. Загаданное слово было", word, ". Удачи!")

cont = "д"
while cont.lower() == "д":
    word = get_word()
    play(word)
    cont = input("Играем еще раз? (д = да / н = нет)\n")
